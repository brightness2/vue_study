 ...
 * @LastEditTime: 2021-08-25 16:42:54
 * @Description:  
-->
<template>
    <div class="level">
        <page-title title="角色管理" />
        <div class="button-group">
            <n-button class="btn" type="info" @click="add">新建角色</n-button>
        </div>
        <!-- 表格 -->
        <vxe-table
            border="inner"
        ...
        >
            ...
            <vxe-column field="count" title="角色成员"></vxe-column>
            <vxe-column field="memo" title="角色描述"></vxe-column>
        ...
        <n-modal v-model:show="showModal" :mask-closable="false">
            <n-card style="width: 600px;" :title="modalTitle" :bordered="false">
                <n-form
                    ref="formRef"
                    :model="formValue"
                    :rules="rules"
                ...
                >
                    ...
                    <n-form-item label="角色名称：" path="role_name">
                        <n-input v-model:value="formValue.role_name"></n-input>
                    </n-form-item>
...
export default defineComponent({
    ...
    setup() {
        ...
        let methods = {
            //加载表格数据
            loadTableData() {
                apiRequest("role/getList").then((res) => {
                    if (res) {
            ...
            //增加
            add() {
                data.modalTitle = "新增角色";
                data.formValue = {};
                data.showModal = true;
                data.formAction = "role/create";
            },
            ...
            //编辑
            rowEditEvent(row) {
                data.modalTitle = "编辑角色";
                data.formAction = "role/update";
                apiRequest("role/get", { role_id: row.role_id }).then((res) => {
                    if (res) {
            ...
            rowDeleteEvent(row) {
                VXETable.modal
                    .confirm("是否删除 " + row.role_name + " 角色？")
                    .then((action) => {
                        if (action === "confirm") {
                            apiRequest("role/delete", {
                                role_id: row.role_id,
                            }).then((res) => {
                                if (res) {
